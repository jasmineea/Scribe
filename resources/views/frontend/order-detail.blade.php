            ...
            <div class="col-12">
                ...
                <table class="table table-bordered mb-5">
                    <tbody>
                        ...
                        <tr><th>Message Preview</th><td align="center"><ul style="width: 100%;list-style: none;">@php if(@$order_detail['campaign_message']&&file_exists(public_path('img/preview/'.@$order_detail['campaign_message']))){ $preview_image=asset('img/preview/'.@$order_detail['campaign_message']); echo("<li style='float: left;padding: 0px 10px;'><img width='100px' style='display:inline;background:url(".asset("storage/".$order_detail['inner_design']).") #f8f8f8;background-position:center;background-size:92%;background-repeat:no-repeat;'  class='model_preview' data-url='".$preview_image."' src='".$preview_image."'></li>"); if(!empty($order_detail['main_design'])){ echo("<li style='float:left;'><img width='100px' class='model_preview' data-url='".asset("storage/".$order_detail['main_design'])."' src='".asset("storage/".$order_detail['main_design'])."'></li>");} }else{ echo$order_detail['campaign_message']; } @endphp </ul></td></tr>
