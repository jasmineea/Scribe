				...
				<table class="table table-bordered mb-5">
					...
					<tbody>
						...
						<tr>
							...
							<td><ul style="width: 100%;list-style: none;">@php if(@$data->campaign_message&&file_exists(public_path('img/preview/'.@$data->campaign_message))){ $preview_image=asset('img/preview/'.@$data->campaign_message); echo("<li style='float: left;padding: 0px 10px;'><img width='100px' class='model_preview'  style='display:inline;background:url(".asset("storage/".$data->inner_design).") #f8f8f8;background-position:center;background-size:92%;background-repeat:no-repeat;' data-url='".$preview_image."' src='".$preview_image."'></li>"); if(!empty($data->main_design)){ echo("<li style='float:left;'><img width='100px' class='model_preview' data-url='".asset("storage/".$data->main_design)."' src='".asset("storage/".$data->main_design)."'></li>");} }else{ echo $data->campaign_message;} @endphp </ul></td>
